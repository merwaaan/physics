...
environnementales, telles que la gravité, et on intègre l'état de tous
les corps. L'algorithme \ref{algoMoteur1} résume cette organisation.
...
avancent d'un pas de temps fixe $\deriv t$ et l'état de ceux subissant
une collision sera inmanquablement corrigé pour correspondre à l'état
qu'ils auraient dû atteindre lors de la collision. blabla

Autre problème de taille : toujours à cause de l'ordre dans lequel les
corps sont ordonnés, la simulation empruntera des chemins différents
selon l'ordre dans lequel les collisions sont détectées. La figure
\ref{issues} nous montre deux corp ainsi que la position qu'ils
atteindront à l'itération suivante si on ignore les collisions. On
peut aisément se rendre compte des conséquences de l'ordre dans lequel
les objets sont traîtés. Si le corps bleu est traîté en premier, il
entrera en collision avec le corps rose, son état sera corrigé, et une
impulsion sera plus tard générée pour séparer les deux corps. Au
final, l'objet bleu sera projeté vers la gauche et le corps rose sera
dévié de sa trajectoire originelle. Par contre, si c'est le corps rose
qui est traîté en premier, il avancera, aucune collision ne sera
détectée, puis l'autre corps avancera à son tour et puisque le corps
rose ne se trouvera plus sur sa trajectoire, rien ne se passera !

\begin{figure}
  \centering
  \subfloat[]{ \input{images/issues.tikz} }
  \caption{Deux issues possibles selon l'ordre de détection des collisions}
  \label{issues}
\end{figure}

[tout est lié au temps collision]
...
\begin{figure}
  \centering
  \input{images/quadtree1.tikz}
  \qquad
  \input{images/quadtree2.tikz}
  \caption{}
  \label{quadtree}
\end{figure}
