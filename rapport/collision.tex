...
Une fois que la collision entre deux solides est confirmée est que
leurs états sont corrigés, il reste à déterminer quels points des
corps entre en collision. Parfois, un seul point de contact peut
exister, comme lors d'une collision entre deux sphères. Plus souvent,
il en existera plusieurs. Un corps rigide est structuré par trois
types d'élément : les sommets, les arêtes et les faces et il est
usuellement admis que seuls les contacts sommet-face et arête-arête
sont nécessaires à nos besoins. Les autres associations d'éléments
peuvent être ramenés à des cas dégénérés. Par exemple, un contact
...
\ref{contacts} illustre une configuration comprenant quatre contacts :
deux contacts sommet/face et deux contacts arête/arête.
...
utilitaires de projection qui déterminent la distance minimum entre
deux éléments. Ici aussi, l'utilisation d'un seuil de tolérance adapté
et en accord avec celui utilisé lors de la correction de l'état
collisionnel est inévitable.

Nous référerons désormais aux informations concernant un point de
contact entre deux corps en tant que contact. Un contact ne s'agit pas
uniquement d'une unique position à laquelle deux corps se touchent, il
doit aussi renseigner sur la direction de la collision, et contiendra
donc une normale. Pour des raisons pratiques, on y adjoindra aussi une
référence à chacun des corps entrant en jeu dans la collision. Il
reste une information supplémentaire à enregistrer : le temps de
contact. Par temps de contact, on ne pense pas à la durée d'un
contact, puisqu'ils sont instantanés dans cette simulation, mais au
moment précis auquel le contact est apparu. On pourrait penser que,
comme le pas de temps de la simulation est fixe, tous les contacts
inexistants au temps $t$ et apparus au temps $t + \deriv t$ auront
tous blablabla

Les contacts sont indispensables au calcul d'une réponse physique
appropriée pour simuler un choc entre deux objets. Après chaque
intégration et si deux corps entrent en collision, une force de
séparation devra être générée pour annuler cette collision, on parle
d'impulsion. Il ne s'agit pas d'une unique force appliquée au centre de
masse de chaque corps, chaque contact engendrera une impulsion. Dans
le cas d'une boîte cubique tombant à plat sur le sol, quatre points de
contact seront détectées : les quatres sommets de la face basse du
cube. \`A chacune de ces positions, une impulsion identique sera
appliquée, et la boîte s'élévera comme si blabla.

La vitesse relative entre deux corps correspond à la composante
blablabla 
