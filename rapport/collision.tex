...
corps entrent en collision. Parfois, un seul point de contact peut
...
peuvent être ramenés à des configurations dégénérés de ces deux
cas. Par exemple, un contact entre une arête et une face peut être
assimilée à deux contacts sommet-face, chaque sommet étant une
extrémité de l'arête. La figure \ref{contacts} illustre une
configuration comprenant quatre points de contact : deux sommet-face
et deux arête-arête.
...
et contiendra donc un vecteur normal au choc. Pour des raisons
...
collisionnel se charge de cibler précisément le moment du
contact. Cette information se révélera utile dans la dernière partie
de ce rapport, lorsque l'on souhaitera s'assurer du bon ordre des
collisions.
...
devra subir autant d'impulsions qu'il existe de contacts entre les
deux corps considérés. Dans le cas d'une boîte cubique tombant à plat
sur le sol, quatre points de contact seront détectées : les quatres
...
devront donc être égales afin de ne pas déséquilibrer l'objet et de
...
simuler : une balle en cahoutchoux aura un $\varepsilon$ elevé tandis
qu'un bloc d'argile aura un $\varepsilon$ proche de zéro.
