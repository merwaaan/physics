...
contiendra toutes les influences que le monde extérieur fait subir aux
corps qu'il contient. On pourra par exemple modéliser, entre autres,
la résistance de l'air ou le roulis irrégulier du vent.
...
l'accélération. C'est la façon dont ces variables évolueront et
s'influenceront entres elles qui déterminera la qualité du rendu du
moteur physique. Dans la première partie de ce compte-rendu, on
précisera donc les méthodes employées pour simuler la dynamique des
corps.
...
\begin{align*}
  &\vec{F}_{A/B} = -\vec{F}_{B/A} \\ &\vec{F}_{A/B} + \vec{F}_{B/A} = 0
\end{align*}
...
système. Ce phénomène sera reproductible dans le moteur physique. [détails]
